...
func transactionKey(hash ogTypes.Hash) []byte {
...
func (da *Accessor) ReadLastStateRoot() ogTypes.Hash {
	...
	return ogTypes.BytesToHash32(rootbyte)
}
...
func (da *Accessor) ReadTxByNonce(addr ogTypes.Address, nonce uint64) types.Txi {
	...
	hash := ogTypes.BytesToHash32(data)
...
func (da *Accessor) ReadIndexedTxHashs(SeqHeight uint64) ([]ogTypes.Hash, error) {
	...
	hashs := make([]ogTypes.Hash, arrLen)
	for i := 0; i < arrLen; i++ {
		var hash ogTypes.Hash
		hash, data, err = ogTypes.UnmarshalHash(data)
		if err != nil {
