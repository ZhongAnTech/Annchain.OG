...
type DkgPartner struct {
	...
	myPublicKey       []byte
	context           *DkgContext
	formerContext     *DkgContext //used for case : context reset , but bft is still generating sequencer
...
func NewDkgPartner(dkgOn bool, numParts, threshold int,
	dkgPulicKeyChan chan kyber.Point, genesisPkChan chan *p2p_message.MessageConsensusDkgGenesisPublicKey, t *term.Term) *DkgPartner {
	c := NewDkgContext(bn256.NewSuiteG2())
...
func (d *DkgPartner) generateDkg() []byte {
	...
	log.WithField("pk", hexutil.Encode(pk[:5])).WithField("len pk", len(d.context.CandidatePublicKey)).WithField("sk", sec).Debug("gen dkg")
	return pk
}
...
func (d *DkgPartner) addPartner(c *tx_types.Campaign) {
	...
	if bytes.Equal(c.PublicKey, d.myAccount.PublicKey.Bytes) {
		...
		log.WithField("id ", d.context.Id).Trace("my id")
	}
	log.WithField("cp ", c).Trace("added context")
