...
func newQueue() *queue {
	...
	return &queue{
		...
		blockTaskPool:    make(map[types.Hash]*types.SequencerHeader),
		blockTaskQueue:   prque.New(),
		blockPendPool:    make(map[string]*fetchRequest),
		blockDonePool:    make(map[types.Hash]struct{}),
		receiptTaskPool:  make(map[types.Hash]*types.SequencerHeader),
...
func (q *queue) Reset() {
	...
	q.blockTaskPool = make(map[types.Hash]*types.SequencerHeader)
	q.blockTaskQueue.Reset()
	q.blockPendPool = make(map[string]*fetchRequest)
	q.blockDonePool = make(map[types.Hash]struct{})

	q.receiptTaskPool = make(map[types.Hash]*types.SequencerHeader)
...
// cache.
func (q *queue) resultSlots(pendPool map[string]*fetchRequest, donePool map[types.Hash]struct{}) int {
...
func (q *queue) ScheduleSkeleton(from uint64, skeleton []*types.SequencerHeader) {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	q.headerTaskPool = make(map[uint64]*types.SequencerHeader)
	q.headerTaskQueue = prque.New()
	q.headerPeerMiss = make(map[string]map[uint64]struct{}) // Reset availability to correct invalid chains
	q.headerResults = make([]*types.SequencerHeader, len(skeleton)*MaxHeaderFetch)
	q.headerProced = 0
	q.headerOffset = from
...
// skeleton.
func (q *queue) RetrieveHeaders() ([]*types.SequencerHeader, int) {
	q.lock.Lock()
	defer q.lock.Unlock()
...
func (q *queue) Schedule(headers []*types.SequencerHeader, from uint64) []*types.SequencerHeader {
	q.lock.Lock()
	defer q.lock.Unlock()
...
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[types.Hash]*types.SequencerHeader, taskQueue *prque.Prque,
	pendPool map[string]*fetchRequest, donePool map[types.Hash]struct{}, isNoop func(*types.SequencerHeader) bool) (*fetchRequest, bool, error) {
	...
	send := make([]*types.SequencerHeader, 0, count)
	skip := make([]*types.SequencerHeader, 0)
...
func (q *queue) DeliverHeaders(id string, headers []*types.SequencerHeader, headerProcCh chan []*types.SequencerHeader) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	if ready > 0 {
		...
		process := make([]*types.SequencerHeader, ready)
...
// reason the lock is not obtained in here is because the parameters already need
// to access the queue, so they already need a lock anyway.
func (q *queue) deliver(id string, taskPool map[types.Hash]*types.SequencerHeader, taskQueue *prque.Prque,
	pendPool map[string]*fetchRequest, donePool map[types.Hash]struct{}, reqTimer metrics.Timer,
	results int, reconstruct func(header *types.SequencerHeader, index int, result *fetchResult) error) (int, error) {
