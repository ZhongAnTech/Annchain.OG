...
func (net *Network) SetFallbackNodes(nodes []*Node) error {
	...
	for _, n := range nodes {
		...
		cpy := *n
		cpy.sha = ogcrypto2.Keccak256Hash(n.ID[:])
...
func (net *Network) Resolve(targetID NodeID) *Node {
	result := net.lookup(ogcrypto2.Keccak256Hash(targetID[:]), true)
...
func (net *Network) Lookup(targetID NodeID) []*Node {
	return net.lookup(ogcrypto2.Keccak256Hash(targetID[:]), false)
}
...
func (q *findnodeQuery) start(net *Network) bool {
	...
	if q.remote == net.tab.self {
		closest := net.tab.closest(ogcrypto2.Keccak256Hash(q.target.Bytes[:]), bucketSize)
...
func (net *Network) handleQueryEvent(n *Node, ev nodeEvent, pkt *ingressPacket) (*nodeState, error) {
	switch ev {
	case findnodePacket:
		target := ogcrypto2.Keccak256Hash(pkt.data.(*Findnode).Target[:])
...
func rlpHash(x msg.MsgpMember) (h types.Hash) {
