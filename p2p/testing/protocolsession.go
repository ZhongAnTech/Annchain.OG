...
import (
	...
	"github.com/annchain/OG/p2p/simulations/adapters"
)
...
func (s *ProtocolSession) trigger(trig Trigger) error {
	...
	go func() {
		log.Debug(fmt.Sprintf("trigger %v (%v)....", trig.Msg, trig.Code))
		errc <- mockNode.Trigger(&trig)
		log.Debug(fmt.Sprintf("triggered %v (%v)", trig.Msg, trig.Code))
	}()
