...
type syncMemBatch struct {
	...
	order common.Hashes          // Order of completion to prevent out-of-order data loss
}
...
func newSyncMemBatch() *syncMemBatch {
	return &syncMemBatch{
		...
		order: make(common.Hashes, 0, 256),
	}
}
...
func (s *Sync) Missing(max int) common.Hashes {
	requests := common.Hashes{}
...
func (s *Sync) Process(results []SyncResult) (bool, int, error) {
	...
	for i, item := range results {
		...
		node, err := decodeNode(item.Hash.ToBytes(), item.Data, 0)
		if err != nil {
...
func (s *Sync) Commit(dbw ogdb.Putter) (int, error) {
	...
	for i, key := range s.membatch.order {
		if err := dbw.Put(key.ToBytes(), s.membatch.batch[key]); err != nil {
			return i, err
		}
	}
