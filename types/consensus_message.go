...
func (m MessageCommonVote) String() string {
	return fmt.Sprintf("bm %s, idv %s", m.BasicMessage, m.Idv)
}

func ( m *MessageCommonVote)SignatureTargets()[]byte {
...
func ( m *MessageProposal)SignatureTargets()[]byte {
	var buf bytes.Buffer
	panicIfError(binary.Write(&buf, binary.BigEndian,m.Value.GetId()))
	panicIfError(binary.Write(&buf, binary.BigEndian, m.HeightRound.Height))
	panicIfError(binary.Write(&buf, binary.BigEndian, m.HeightRound.Round))
...
func (s *SequencerProposal) String() string {
	return fmt.Sprintf("seqProposal") + s.RawSequencer.String()
}
...
func (s SequencerProposal) Equal(o Proposal) bool {
	...
	return s.Sequencer().GetTxHash() == v.Sequencer().GetTxHash()
}
